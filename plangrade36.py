...
def calculate_target_gpa():
    ...
    try:
        ...
        current_gpa = float(
            target_current_gpa.get()
        )

        completed_credit = float(
            target_completed_credit.get()
        )

        remaining_credit = float(
            target_remaining_credit.get()
        )

        target_gpa = float(
            target_gpa_entry.get()
        )
...
def calculate_plan():
    ...
    try:
        ...
        try:
            ...
            target = float(
                plan_target_entry.get()
            )

            if planned_gpa >= target:

                plan_status_label.config(
                    text="✓ แผนนี้สามารถถึง GPA เป้าหมาย",
                    fg=GREEN
                )

            else:

                difference = target - planned_gpa

                plan_status_label.config(
                    text=f"⚠ ยังต่ำกว่าเป้าหมาย {difference:.2f}",
                    fg=ORANGE
                )
